...
export default async function handler(req) {
  ...
  try {
    ...
    // Set a timeout for the request with longer duration - increased from 300s to 360s
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 360000); // 6 minute timeout (increased from 5m)
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        return new Response(JSON.stringify({ 
          ...
          timeoutMs: 360000  // Updated timeout value
        }), {
